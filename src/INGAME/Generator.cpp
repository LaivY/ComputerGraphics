...
void cross_maze(std::vector<Obstacles>& obs, float z)
{
	...
	Obstacles o1(Cube(glm::vec3(0.0, 0.8, z), 0.7));


	Obstacles o2(Cube(glm::vec3(-1.0, 0.8, z - 1.8), 0.7));
	Obstacles o3(Cube(glm::vec3(1.0, 0.8, z - 1.8), 0.7));
	Obstacles o4(HCube(glm::vec3(0.0, 0.3, z-0.9), 0.2,0.01));
...
void side_maze(std::vector<Obstacles>& obs, std::vector<Item>& item, float z)
{
	for (int i = 0; i < 3; i++)
	{
		glm::vec3 lPos = { -1.0, 0.8, z - 1.8 * i };
		Obstacles left = Cube(lPos, 0.7);

		glm::vec3 rPos = { 1.0, 0.8, z - 1.8 * i };
		Obstacles right = Cube(rPos, 0.7);
...
void zige(std::vector<Obstacles>& obs, std::vector<Item>& item, float z)
{
	...
	Item a = Heal(glm::vec3(-0.8 * reflect, 0.3, z - 2.8));
	item.push_back(a);
}
...
void jump_3(std::vector<Obstacles>& obs, std::vector<Item>& item, float z)
{
	...
	Obstacles o1 = Cube(glm::vec3(-1.0, 0.8, z - 0.8), 0.7);
	Obstacles o2 = Cube(glm::vec3(-0.0, 0.4, z - 0.8), 0.4);
	Obstacles _o2 = Cube(glm::vec3(-0.0, 0.3, z - 0.4), 0.2);
	Obstacles o3 = Cube(glm::vec3(1.0, 0.8, z - 0.8), 0.7);

	Obstacles o4 = Cube(glm::vec3(-0.2, 0.8, z - 2.2), 0.7);
	Obstacles o5 = Cube(glm::vec3(0.2, 0.8, z - 2.2), 0.7);
